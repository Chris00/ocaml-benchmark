...
let bigdata =
  ...
  for _ = 1 to (size / 10) - 1 do
    Buffer.add_string buf "012345678 "
  done;
...
let () =
  (* Print out the results of the functions to doublecheck that they
     work as we intend. *)
  let a, b = pcre_match bigdata in
  printf "Pcre matches: %s %s\n" a b;
  let a, b = str_match bigdata in
  printf "Str  matches: %s %s\n" a b;
  print_newline ();

  let res =
    throughputN ~repeat:5 5
      [ "pcre match", pcre_match, bigdata; "str match", str_match, bigdata ]
  in
  print_newline ();
  tabulate res
