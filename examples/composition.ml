    ...
    if i <= 0 then
      ...
      let p =
        if c = 0 then
          ...
          reverse (Random.int n) (Random.int n)
        else
          (* c = 2 *)
          splice (Random.int n) (Random.int n) (Random.int n)
      in
...
let () =
  ...
  let do_f () =
    let f = List.fold_left (fun f f0 k -> f0 (f k)) (fun k -> k) p_f in
    Array.map f v
  and do_v () =
    snd
      (List.fold_left
  ...
  let res = throughputN ~repeat:3 5 [ "fun", do_f, (); "vec", do_v, () ] in
  tabulate res;
  print_gc res
