...
let make n =
  ...
  let tms = Unix.times () in
  {
    ...
    cutime = tms.Unix.tms_cutime;
    cstime = tms.Unix.tms_cstime;
    iters = n;
    minor_words;
    major_words;
    promoted_words;
...
let add a b =
  {
    ...
    cutime = a.cutime +. b.cutime;
    cstime = a.cstime +. b.cstime;
    iters = Int64.add a.iters b.iters;
...
let sub a b =
  {
    ...
    cutime = a.cutime -. b.cutime;
    cstime = a.cstime -. b.cstime;
    iters = Int64.sub a.iters b.iters;
    minor_words = a.minor_words -. b.minor_words;
    major_words = a.major_words -. b.major_words;
    promoted_words = a.promoted_words -. b.promoted_words;
  }
...
let to_string ?(style = Auto) ?(fwidth = 5) ?(fdigits = 2) (b : t) : string =
  let pt = cpu_process b and ct = cpu_childs b in
  let style =
    if style = Auto then
      if ct > 1e-10 then
        All
  ...
  | All ->
    sprintf
      "%s WALL (%s usr %s sys + %s cusr %s csys = %s CPU, minor = %s, major = \
       %s)%s"
...
let rec string_of_time t =
  if t = 0 || t = 1 then
    ...
    "1m " ^ string_of_time (t - 60)
  else
    string_of_int (t / 60) ^ "m " ^ string_of_time (t mod 60)
...
let merge (l1 : samples) (l2 : samples) =
  ...
  let rec do_merge acc l1 l2 =
    ...
    | ((n1, t1) as d1) :: tl1, ((n2, t2) as d2) :: tl2 ->
      ...
      if sgn = 0 then
        do_merge ((n1, t1 @ t2) :: acc) tl1 tl2
      else if sgn < 0 then
        do_merge (d1 :: acc) tl1 l2
      else
        do_merge (d2 :: acc) l1 tl2
  in
...
let runloop n_iters n f x =
  ...
  let n1 = Int64.to_int n' and n0 = Int64.to_int (Int64.rem n max_iter) in
...
let print_run out ?(min_count = 4L) ?(min_cpu = 0.4) ~style ?fwidth ?fdigits b =
  ...
  then
    out.print_indent "(warning: too few iterations for a reliable count)\n"

let latency n out ?min_count ?min_cpu ~style ?fwidth ?fdigits ~repeat _name f x
    =
  let rec loop nrep acc =
    if nrep < 1 then
      ...
      Gc.full_major ();
      Gc.compact ();
      (* Reclaim memory to avoid undue GC during the test. *)
...
let throughput tmin out ?min_count ?min_cpu ~style ?fwidth ?fdigits ~repeat
    _name f x : t list =
  ...
  let rec run_test nmin niter bm_init total_wall =
    ...
    if tn >= tmin then (
      ...
      (* FIXME *)
      let n =
        Int64.of_float (((tmin /. tn) -. 1.) *. Int64.to_float bm.iters)
      in
      run_test nmin (max nmin n) bm total_wall
    )
  in
    ...
    if nrep < 1 then
      ...
      let wall_estim =
        if wall > wall_estim +. 60. then (
          out.print_indent
            ("(Estimated time for subsequent runs: " ^ string_of_time wall
           ^ ")\n");
          wall
        ) else
          wall_estim
  ...
  let rec estimate_niter nmin n tn wall =
    if tn >= tpra then (
      ...
      if wall_estim >= 60. then
        out.print_indent
          ("(Estimated time for each run: " ^ string_of_time wall_estim ^ ")\n");
      ...
      let n =
        ...
        if new_tn > 1.2 *. tn then
          new_n
        else
          Int64.of_float ((1.1 *. Int64.to_float n) +. 1.)
        (* FIXME *)
      in
      estimate_niter nmin n new_tn new_wall
    )
  in
    ...
    if tn < 0.1 then (
      ...
      if takes_long then
        if total_wall >= 120. then (
          ...
          failwith
            (sprintf
               "Benchmark.throughputN: wall time is %g while CPU time is %g.  \
                Do you use \"sleep\"?"
               total_wall tn)
        ) else if previous_took_long then
          out.print "."
        else
          out.print_indent
            "(Determining how many runs to perform, please be patient...";
      let twice_n = Int64.shift_left n 1 in
      min_iter twice_n ~takes_long total_wall
    ) else (
      if previous_took_long then out.print ")\n";
      if tn < tmin then
        estimate_niter n n tn wall (* tn > 0.1 *)
      else (
        (* minimal [n] good for [tmin], use the above measurement
           for the first run. *)
        print_run out ?min_count ?min_cpu ~style ?fwidth ?fdigits bm;
...
let testN ~test default_f_name ?min_count ?min_cpu ~style ?fwidth ?fdigits
    ~repeat funs =
  let length_name =
    List.fold_left (fun m (n, _, _) -> max m (String.length n)) 0 funs
  in
  let result_of (name, f, x) =
    ...
    let out =
      if style = Nil then
        ...
        make_printer (length_name + 2)
    in
    let bm =
      test out ?min_count ?min_cpu ~style ?fwidth ?fdigits ~repeat name f x
    in
    name, bm
  in
...
let latencyN ?min_cpu ?(style = Auto) ?fwidth ?fdigits ?(repeat = 1) n funs =
  ...
  if style <> Nil then
    printf "Latencies for %Ld iterations of %s%s:\n%!" n (string_of_names funs)
      (if repeat > 1 then
         sprintf " (%i runs)" repeat
       else
         "");
  testN ~test:(latency n)
    (sprintf "[run %Ld times]" n)
...
let throughputN ?min_count ?(style = Auto) ?fwidth ?fdigits ?(repeat = 1) n funs
  ...
  if style <> Nil then
    printf "Throughputs for %s%s running%s for at least %g CPU second%s:\n%!"
      ...
      (if List.length funs > 1 then
         " each"
       else
         "")
      (if repeat > 1 then
         sprintf " %i times" repeat
       else
         "")
      tmin
      (if n > 1 then
         ...
         "");
  testN ~test:(throughput tmin)
    (sprintf "[run > %3.1g secs]" tmin)
...
let betai_cf x a b =
  ...
  let rec lentz m c d f =
    ...
    let delta = c *. d in
    let f = f *. delta in
    if abs_float (delta -. 1.) < betai_cf_eps then
      f
    else
      lentz (m +. 1.) c d f
  in
...
let different_rates significance n1 r1 s1 n2 r2 s2 =
  ...
  if n1 = 1 && n2 = 1 then
    true
  (* no info about distribution, assume
     they really are. *)
  else (
...
let string_of_rate display_as_rate =
  ...
  fun confidence n r s : (string * string) ->
    ...
    let a, err =
      ...
      (* Taylor of order 1 of 1/r *)
    in
    let p prec =
      if sigma < 1e-15 then
        sprintf " %.*f%s" prec a per_sec, ""
      else
        sprintf " %.*f+-" prec a, sprintf "%.*f%s" prec err per_sec
    in
    if a >= 100. then
      p 0
    else if a >= 10. then
      ...
      p 3
    else if sigma < 1e-15 then
      sprintf " %g%s" a per_sec, ""
    else
      sprintf " %g+-" a, sprintf "%g%s" err per_sec
...
let tabulate ?(no_parent = false) ?(confidence = 0.95)
  ...
  let display_as_rate =
    let _, _, r, _ = List.nth rates (len / 2) in
    r > 1.
  in
  (*
   * Compute rows
    ...
    let make_col j (_col_name, col_n, col_rate, col_s) =
      let ratio =
        if i = j || is_nan row_rate || is_nan col_rate then
          ...
          if
            p = 0.
            || different_rates (1. -. confidence) row_n row_rate row_s col_n
                 col_rate col_s
          then
            sprintf " %.0f%%" p
          else
      ...
      ratio
    in
    row_name :: ra :: ra_err :: list_mapi make_col rates
  in
  ...
  and stretch_min i min_width total =
    (* try to stretch col [i] *)
    if total < 80 then
      if i < len && col_width.(nth.(i)) = min_width then (
        col_width.(nth.(i)) <- col_width.(nth.(i)) + 1;
        stretch_min (i + 1) min_width (total + 1) (* stretch next col? *)
      ) else
        stretcher (min_width + 1) total (* try again to stretch *)
  in
...
let print_gc (results : (_ * t list) list) : unit =
  ...
  let rows =
    List.map2
      (fun (name, minor) ((_, major), (_, promoted)) ->
        [
          name;
          string_of_bytes minor;
          string_of_bytes major;
          string_of_bytes promoted;
        ])
      minor_rates
      (List.combine major_rates promoted_rates)
  in
  ...
  List.iter
    (List.iteri (fun i col ->
         col_width.(i) <- max col_width.(i) (String.length col)))
    rows;
...
module Tree = struct
  ...
  *)
  let rev_parse_path s =
    ...
    let name =
      if !i0 = 0 then
        ...
        String.sub s !i0 (String.length s - !i0)
    in
    name :: !l
  ...
  type benches =
    | Single of samples Lazy.t
    | Pair of benches * benches

  let merge_benches_opt b1 b2 =
    match b1, b2 with
    | None, b | b, None -> b
  ...
  let rec benches_iter benches ~f =
    match benches with
    | Single b -> f b
    | Pair (b1, b2) ->
      benches_iter b1 ~f;
  ...
  let is_empty (Tree (b, m)) = b = None && SMap.is_empty m
  ...
  let prefix path t = List.fold_right (fun n t -> name t n) path t
  let ( @>> ) n t = List.fold_left name t (rev_parse_path n)
  ...
  let rec print_tree_map fmt m = SMap.iter (print_tree_path fmt) m
  ...
  let print fmt (Tree (b, m)) =
    ...
    | Some b ->
      let n = number_of_benches b in
      Format.fprintf fmt "%i benchmark%s at root" n
        (if n > 1 then
           "s"
         else
           ""));
  ...
  let rec filter path (Tree (b, m) as t) =
    match path with
    | [] -> t
    | [ "" ] ->
      ...
      Tree (b, SMap.empty)
    | "" :: tl ->
      (* skip empty component NOT at the end, skip *)
      filter tl t
    | "*" :: tl ->
  ...
  let rec run_all ~with_gc fmt is_previous_output rev_path (Tree (b, m)) =
    let is_previous_output =
      run_bench_path ~with_gc fmt is_previous_output rev_path b
    in
    SMap.fold
      (fun name t is_out -> run_all ~with_gc fmt is_out (name :: rev_path) t)
    ...
    let options =
      Arg.align
        [
          ...
          ( "--tree",
            Arg.Unit (fun () -> st.print_tree <- true),
            " print the tree" );
          "--gc", Arg.Unit (fun () -> st.with_gc <- true), " print GC stats";
          ( "--no-gc",
            Arg.Unit (fun () -> st.with_gc <- false),
            " do not print GC stats" );
        ]
    in
    st, options
